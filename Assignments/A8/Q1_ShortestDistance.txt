...
{
    queueFront = getFront(q);
    if(queueFront==w)
    {  queueFront = parent[w-1];

       while(queueFront!=v)
       {
          depth++;
          queueFront = parent[queueFront-1];
          //printf("%d debug\n", queueFront);
       }
    ...
    dequeue(&q);
     temp = g.list[queueFront - 1];
    while(temp != NULL) {
    if (!visited[(temp->vertex) -1])
    {

       visited[(temp->vertex) -1] = 1;
       parent[(temp->vertex) -1]=queueFront;
       //printf("now the parent is %d\n", parent[(temp->vertex) -1]);
